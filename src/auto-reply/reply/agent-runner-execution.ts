...
export async function runAgentTurnWithFallback(params: {
  ...
  if (errorForOverflowCheck && !hasPayloadText && isContextOverflowError(errorForOverflowCheck)) {
    if (finalEmbeddedError) params.replyOperation?.fail("run_failed", finalEmbeddedError);
    return {
      kind: "final",
      payload: markAgentRunFailureReplyPayload({
