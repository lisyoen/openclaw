...
export type GetReplyOptions = {
  ...
  /** P2.26 (2026-05-25): agent 턴 1회 완료 직후·payload dispatch 직전 1회 발화. stopReason+payload 신호로 빈응답 실패 검출(Gemma 4 NVFP4 tool_call_parser 간헐 실패: content=[] + stopReason="toolUse"). */
  onAgentRunEnd?: (info: {
    runId: string;
    stopReason?: string;
    payloadCount: number;
