...
const TIME_PATTERNS: RegExp[] = [
  ...
  // P2.30: "지난/저번 N요일" 또는 단독 "N요일" — "지난 일요일에 모였을 때..."
  // 류 표현이 looksLikeNaturalMemoRequest=true 가 되도록. recall.sh 의
  // TIME_WEEKDAY_PAT 과 동일 의미.
  /(?:지난|저번)\s*(?:일|월|화|수|목|금|토)요일|[일월화수목금토]요일/,
];
...
// Stock v2026.6.8 baseline: PluginHookMessageReceivedEvent.senderId 는 top-level
// (hook-message.types.ts:104), PluginHookMessageContext.senderId 도 top-level
// (hook-message.types.ts:46). stock #87144 group prompt metadata sanitize 로
// event.metadata.senderId 가 group inbound 에서 비워질 수 있으므로 top-level 을
// 우선 채택하고 metadata 는 보조, sessionKey direct:<id> 는 DM 전용 폴백.
export function extractSenderUserId(
  event: { senderId?: unknown; metadata?: Record<string, unknown> } | undefined | null,
  ctx: { senderId?: unknown; sessionKey?: string } | undefined | null,
): string | undefined {
  const eventTop =
    event && typeof event === "object" ? (event as { senderId?: unknown }).senderId : undefined;
  if (typeof eventTop === "string" && eventTop) return eventTop;
  if (typeof eventTop === "number" && Number.isFinite(eventTop)) return String(eventTop);
  const ctxTop =
    ctx && typeof ctx === "object" ? (ctx as { senderId?: unknown }).senderId : undefined;
  if (typeof ctxTop === "string" && ctxTop) return ctxTop;
  if (typeof ctxTop === "number" && Number.isFinite(ctxTop)) return String(ctxTop);
  ...
  const sk =
    ctx && typeof ctx === "object" ? (ctx as { sessionKey?: string }).sessionKey : undefined;
...
// Stock v2026.6.8 recompile (2026-06-18): PluginHookMessageContext (message_received)
// 에 agentId 가 없어 sessionKey 의 leading `agent:<id>:` 세그먼트에서 파싱한다.
// before_prompt_build (PluginHookAgentContext) 와 before_tool_call (PluginHookToolContext)
// 는 ctx.agentId 를 직접 노출하므로 이 헬퍼 불필요.
function sessionKeyToAgentId(sessionKey: string | undefined): string | undefined {
  if (typeof sessionKey !== "string" || !sessionKey) return undefined;
...
export default (api: OpenClawPluginApi) => {
  ...
  api.on("message_received", (event, ctx) => {
    // Stock v2026.6.8 recompile (2026-06-18): PluginHookMessageContext (the ctx
    // type stock binds to message_received) does NOT expose `agentId`. Derive
    // it from sessionKey's leading `agent:<id>:` segment. before_prompt_build /
    // before_tool_call below keep direct ctx.agentId because their ctx types
    // (PluginHookAgentContext / PluginHookToolContext) carry it natively.
    const sessionKey = ctx.sessionKey;
    const agentId = sessionKeyToAgentId(sessionKey);
  ...
  api.on("before_tool_call", (event, ctx) => {
    // Stock v2026.6.8: ctx is PluginHookToolContext — exposes agentId/sessionKey directly.
    const agentId = ctx.agentId;
    if (agentId !== "gemma") {
